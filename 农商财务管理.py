...
# 转账额度
def bank_transedu(outputaccount,outputmoney):
    account_edu = findByAccount(outputaccount)

    if bank[account_edu]["acttype"] == 1:
        if int(outputmoney) <= 50000:
            return True
        else:
            return False
    elif bank[account_edu]["acttype"] == 2:
        if int(outputmoney) <= 20000:
            return True
        else:
            return False
...
def bank_transformMoney(outputaccount,inputaccount,outputpassword,outputmoney):
    ...
    uname1 = findByAccount(inputaccount)
    if uname != None and outputaccount != None:
        if uname1 != None and outputaccount != None:
            if outputpassword == bank[uname]["password"]:
                return 0
            else:
                return 2
        else:
...
def transformMoney():
    ...
    t = bank_transedu(output,outputmoney)
    if f == 1:
        print("转出或转入的账号不存在！")
    elif f == 2:
        print("输入密码错误！")
